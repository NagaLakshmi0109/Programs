...
def searchId():
	temproraryRecordId = input(f"Enter {fieldNames[0]} to {operation}: ")
	returnedValue = cursor.execute(f"SELECT {fieldNames[0]} FROM {table} WHERE {recordId} = '{temproraryRecordId}'").fetchall()
	if returnedValue == []:
		return None
	else:
		return temproraryRecordId
...
def searchRecord():
	...
	if temproraryRecordId == None:
		...
		selectQuery = f"SELECT {fields} FROM {table} WHERE {recordId} = '{temproraryRecordId}'"
